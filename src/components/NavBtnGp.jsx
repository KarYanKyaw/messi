        ...
        {navBtnData.map((el) => {
          ...
          return (
            <NavLink
              key={el.name}
              to={`${
                el.name.toLowerCase() === "home"
                  ? "/"
                  : `/${el.name.toLowerCase()}`
              }`}
              className={`group flex justify-center items-center border w-11 h-11 border-slate-600 p-2 rounded-full hover:bg-slate-700 relative bg-transparent  duration-500`}
            >
              {el.svg}
